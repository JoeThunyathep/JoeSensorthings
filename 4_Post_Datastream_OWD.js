    ...
    var IdObsProp_First = 33; //28
    var IdObsProp_Last  = 34; //34
    // ============================ Define Variable =========================
    var datastreamBody = []; 
    ...
    function Post_Sensorthings() {
        ...
        for (let o_id = IdObsProp_First; o_id <= IdObsProp_Last; o_id++) { /*Whole range 1 to 16*/
                ...
                if (o_id == 28) { 
                    ...
                    DS_Name         = `OpenWeatherData: ${CoreDS_Name}`,
                    ...
                    UoM_Name        = "Kelvin",
                    UoM_Definition  = "https://en.wikipedia.org/wiki/Kelvin",
                    UoM_Symbol      = "k"    
                }
                if (o_id == 29) { 
                ...
                if (o_id == 31) { 
                    CoreDS_Name     = "WindSpeed"
                    DS_Name         = `OpenWeatherData: ${CoreDS_Name}`,
                    DS_Description  = `Datastream for recording ${CoreDS_Name}`,
                    DS_obType       = "http://www.opengis.net/def/observationType/OGC-OM/2.0/OM_Measurement",
                    UoM_Name        = "meter/sec",
                    UoM_Definition  = "https://en.wikipedia.org/wiki/Metre_per_second",
                    UoM_Symbol      = "m/s"    
                }
                if (o_id == 32) { 
                    CoreDS_Name     = "WindDegree"
                    DS_Name         = `OpenWeatherData: ${CoreDS_Name}`,
                    DS_Description  = `Datastream for recording ${CoreDS_Name}`,
                    DS_obType       = "http://www.opengis.net/def/observationType/OGC-OM/2.0/OM_Measurement",
                    UoM_Name        = "degree",
